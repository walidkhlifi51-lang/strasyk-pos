...
export function useOrderPayment({ 
  ...
  const handleCreateOrUpdateOrder = useCallback(async (paymentInfo) => {
    console.log('🔍 [useOrderPayment] Début création commande:', {
      currentOrder: currentOrder ? {
        articles: currentOrder.articles?.length,
    ...
    try {
      ...
      if (!isOnline) {
        ...
        const { orderType, table, notes, customer } = currentOrder;
        ...
        const totalForPayment = calculateOrderTotal(currentOrder);
        
        const offlineTaxSummary = computeTaxSummaryFromArticles(orderArticles, totalForPayment);
      ...
      if (currentOrder.editingInfo?.payee && Array.isArray(currentOrder.articles) && currentOrder.articles.some(item => item.product_id === 'frais_livraison_conversion')) {
        ...
        const orderPayload = withTenant({
          ...
          delivery_address: finalOrderType === 'livraison' ? (finalCustomer?.selectedAdresse || finalCustomer?.adresse || null) : null,
          articles: orderArticles,
          total_ht: taxSummary.totalHt,
          total_tva: taxSummary.totalTva,
          total_ttc: finalTotalTTC,
          statut: newStatus,
          mode_paiement: currentOrder.editingInfo?.payee
        ...
        if (currentOrder.editingInfo) {
          ...
          savedOrder = await appClient.entities.Order.create({
            ...orderPayload,
            created_date: new Date().toISOString(),
          });
        }
  ...
  }, [queryClient, currentOrder, toast, selectedCustomer, nextNumeroCaisse, cagnotteRule, withTenant, currentTenant, filterByTenant, currentUser, isOnline, workingDate, getCachedData, cacheData, addPendingOperation, setSelectedCustomer]);
